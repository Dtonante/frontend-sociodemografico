...
const CompEditarEstructuraOrganizacional = () => {
  ...
  const actualizar = async (e) => {
    ...
    const camposObligatorios = [
      {
        nombre: "var_nombreArea",
        valor: var_nombreArea,
      },
    ];
        ...
        <CardContent sx={{ padding: "30px" }}>
          <form onSubmit={actualizar}>
            ...
            <TextField
              ...
              onKeyPress={(event) => handleKeyPress(event, "var_nombreArea")}
              onChange={(e) => {
                const valor = e.target.value;
                setVar_nombreArea(valor);
                // Validar si el campo está vacío al cambiar el valor
                validarCampoRequerido(valor, setErrorArea);
              }}
              error={errorArea} // Mostrar borde rojo si hay error
              helperText={errorArea ? "Este campo es obligatorio" : ""} // Mensaje de error
              fullWidth
              sx={{ mb: 2 }}
              InputProps={{
                sx: {
