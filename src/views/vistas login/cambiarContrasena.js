  ...
  const handleCambiarContrasena = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("Error al cambiar la contraseña:", response.data.message); 
      }
    } catch (error) {
      setError("Error al cambiar la contraseña.");
          ...
          <Card variant="outlined" style={{ backgroundColor: "#F2F2F2", borderColor: "#202B52" }}>
            <CardContent>
              <form onSubmit={handleCambiarContrasena}>
                <Typography variant="h5" align="center" color="primary" gutterBottom>  Cambiar Contraseña </Typography>
                <TextField label="Contraseña Nueva"  name="var_contrasenaNueva" type="password" variant="outlined" fullWidth  margin="normal" value={formData.var_contrasenaNueva} onChange={handleChange} style={{ backgroundColor: "#F2F2F2" }} />
                <TextField label="Confirmar Contraseña" name="var_contrasenaConfirmada"  type="password" variant="outlined" fullWidth  margin="normal" value={formData.var_contrasenaConfirmada} onChange={handleChange}  style={{ backgroundColor: "#F2F2F2" }} />
                {error && (
                ...
                <Box mt={2}>
                  <Button type="submit" variant="contained" style={{ backgroundColor: "#202B52" }} fullWidth > Cambiar Contraseña </Button>
                </Box>
                <Box mt={2}>
                  <Button variant="contained" style={{ backgroundColor: "#00A5CE" }} fullWidth  onClick={atras} > Atrás </Button>
                </Box>
              </form>
            </CardContent>
          </Card>
